...
from literature_digest.query import SourceQuery, parse
...
def source_query() -> SourceQuery:
    return SourceQuery(
        term_name="sports-nutrition",
        parsed=parse('TITLE-ABS-KEY("sports nutrition")', term_name="sports-nutrition"),
        area_slug="sports-nutrition",
    )
...
def test_search_loads_object_and_list(tmp_path: Path, source_query: SourceQuery) -> None:
    area_dir = tmp_path / source_query.area_slug
    ...
    articles = LocalSource(_settings(tmp_path)).search(source_query, None)
    ...
    assert all(a.area_slug == source_query.area_slug for a in articles)


def test_search_missing_area_dir_returns_empty(tmp_path: Path, source_query: SourceQuery) -> None:
    assert LocalSource(_settings(tmp_path)).search(source_query, None) == []
