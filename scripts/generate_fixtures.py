...
TERM_THEMES: dict[str, dict[str, list[str]]] = {
    "game_model": {
        "concept": ["team game model", "tactical principles", "positional play framework",
                    "playing-model adherence", "structured possession model"],
        "metric": ["expected threat", "possession-chain value", "build-up progression rate",
                   "positional compactness index"],
    },
    "tracking_data": {
        "concept": ["optical tracking data", "GPS-derived positional data",
                    "player tracking streams", "multi-camera tracking", "broadcast tracking"],
        "metric": ["pitch-control surface", "off-ball run detection", "synchronised movement",
                   "high-speed running distance"],
    },
    "style_of_play": {
        "concept": ["style of play", "playing-style fingerprint", "team behavioural signature",
                    "tactical identity", "possession vs direct style"],
        "metric": ["passing network centrality", "vertical progression ratio",
                   "pressing intensity (PPDA)", "field-tilt share"],
    },
    "spatiotemporal": {
        "concept": ["spatiotemporal patterns", "spatiotemporal pitch occupation",
                    "space-time interaction", "collective movement dynamics",
                    "spatiotemporal pressing traps"],
        "metric": ["Voronoi space control", "temporal synchronisation index",
                   "space-generation rate", "convex-hull surface area"],
    },
}
...
FIRST = ["Alvarien", "Boskovic", "Chidera", "Dufresne", "Enomoto", "Farkas",
         "GrØnvold", "Haddad", "Ishikawa", "Jovanetti", "Kristensen", "Laoutaris"]
LAST = ["Mbeki", "Novak", "Oyelaran", "Petrov", "Quintero", "Rasmussen",
        "Sundqvist", "Tanaka", "Ubeda", "Virtanen", "Wojcik", "Xu"]
JOURNALS = ["Journal of Sports Analytics", "International Journal of Performance Analysis in Sport",
            "Journal of Sports Sciences", "Frontiers in Sports Modelling",
...
def generate(area: str, count: int, seed: int,
             search_terms_dir: Path, fixtures_dir: Path) -> None:
    ...
    for term in terms:
        ...
        for i in range(count):
            ...
            papers.append({
                ...
                "year": year,
                "pub_date": f"{year}-{rng.randint(1, 12):02d}-{rng.randint(1, 28):02d}T00:00:00Z",
                # Forward-compat with subtask 05 (ignored by the model today):
                "matched_terms": [term],
                "sources": ["fixture"],
            })
        (out_dir / f"{term}.json").write_text(
